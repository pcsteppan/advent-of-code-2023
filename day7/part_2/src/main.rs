...
impl Card {
    fn from_str(str: &str, score_value: usize) -> Card {
        ...
        let mut distinct_count = distinct_card_set.len();
        let mut most_frequent_count = counts
            .clone()
            .into_iter()
            .filter(|(k, _)| k != &'J')
            .map(|(_, v)| v)
        ...
        let j_count = counts.get(&'J').unwrap_or(&0).to_owned();

        if j_count > 0 && j_count < 5 {
            distinct_count -= 1;
            most_frequent_count += j_count;
        }
