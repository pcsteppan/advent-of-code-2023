...
fn part1() {
    let content = std::fs::read_to_string("./src/input.txt").expect("could not read file");
    ...
    let sum: u32 = content
        ...
        .map(|l| {
            ...
            head.to_digit(10).unwrap() * 10 + tail.to_digit(10).unwrap()
        })
        .sum();
...
fn part2() {
    let content = std::fs::read_to_string("./src/test.txt").expect("could not read file");
