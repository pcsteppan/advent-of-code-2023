use std::{fs, ops::Range, collections::HashMap, str::FromStr };
...
impl System {
    fn get_seed_locations(self: &Self) -> Vec<i64> {
        ...
        while self.maps.get(curr_src).is_some() {
            ...
            seed_transforms = seed_transforms.into_iter().map(|seed| curr_map.get(seed)).collect();
        ...
        for line in s.lines() {
            if line.contains("seeds") {
                ...
                if is_part_2 {
                    ...
                    seeds = line.split_once(": ").unwrap().1.split_whitespace().map(|n| n.parse().unwrap()).collect();
                }
            } else if line.contains("map") {
                // e.g., 'soil-to-fertilizer map:'
                (curr_src, curr_dest) = line.split_once(" ").unwrap().0.split_once("-to-").unwrap();
                maps.insert(curr_src.to_string(), Map {
